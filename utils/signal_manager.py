...
class SignalType(Enum):
    ...
    # 流程图相关信号
    FLOW_NODE_CREATED = "flow.node_created"
    ...
    def connect(self, signal_type: SignalType, receiver: Callable, sender: Any = None) -> None:
        ...
        with self._lock:
            ...
            if signal_obj:
                ...
                print(f"❌ 信号类型不存在: {signal_type.value}")
    ...
    def disconnect(self, signal_type: SignalType, receiver: Callable, sender: Any = None) -> None:
        ...
        with self._lock:
            ...
            if signal_obj:
                ...
                # 从接收器列表中移除
                receivers_list = self._receivers[signal_type.value]
                self._receivers[signal_type.value] = [
    ...
    def send(self, signal_type: SignalType, sender: Any, data: Any = None) -> list:
        ...
        with self._lock:
            ...
            if signal_obj:
                ...
                print(f"📡 发送信号: {signal_type.value} from {sender.__class__.__name__}")
                results = signal_obj.send(sender, signal_data=signal_data)
                return [result[1] for result in results]  # 返回接收器的返回值
            else:
                print(f"❌ 信号类型不存在: {signal_type.value}")
                return []
...
# 装饰器支持
def signal_receiver(signal_type: SignalType, sender: Any = None):
    """
    信号接收器装饰器
