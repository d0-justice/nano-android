...
class MainWindow:
    def __init__(self, page: ft.Page):
        ...
        self.element_inspectors = []  # 存储ElementInspector实例
        self.hierarchies = []  # 存储Hierarchy实例
        self.flow_nodes = []
    ...
    def _on_window_event(self, e):
        ...
        if e.data == "close":
            print("窗口正在关闭，清理资源...")
            # 清理所有DeviceView实例
            for device_view in self.device_views:
    ...
    def _handle_screenshot_capture(self):
        ...
        print(f"device_views列表长度: {len(self.device_views)}")
        print(f"device_screenshots列表长度: {len(self.device_screenshots)}")
        
        # `键触发截图功能
        screenshot_count = 0
        for i, device_view in enumerate(self.device_views):
            ...
            if hasattr(device_view, 'client') and device_view.client and device_view.client.alive:
                ...
                if hasattr(device_view, 'current_frame') and device_view.current_frame is not None:
                    ...
                    for j, screenshot_view in enumerate(self.device_screenshots):
    ...
    def _on_keyboard(self, e: ft.KeyboardEvent):
        ...
        # 优先检查`键截图功能
        if e.key == "F1" and self.listview_page_state == 1:
            self._handle_screenshot_capture()
        # 检查F1-F4键，更新ListView翻页状态
        elif e.key == "F1":
            ...
            print(f"ListView翻页状态更新为: {self.listview_page_state}")
            self._scroll_to_position(e.key)
        elif e.key in self.SCROLL_POSITIONS:
            self._scroll_to_position(e.key)
        else:
            # 将键盘事件转发给当前活跃的设备视图
            for device_view in self.device_views:
    ...
    def _create_ui_components(self):
        ...
        def button_clicked(e):
            self.page.add(ft.Text("按钮被点击了!", color=ft.Colors.GREEN))
    ...
    def _create_list_item(self, index):
        ...
        if index == 0:  # 第一个item使用DeviceView
            ...
            self.device_views.append(content_area)
            ...
            self.device_screenshots.append(content_area)
            ...
            # 将ElementInspector实例添加到列表中
            self.element_inspectors.append(content_area)
            ...
            if len(self.element_inspectors) > 0:
                # 获取最新的ElementInspector实例
                current_inspector = self.element_inspectors[-1]
                content_area.set_element_select_callback(current_inspector.set_selected_element)
            
            # 将Hierarchy实例添加到列表中
            self.hierarchies.append(content_area)
    ...
    def _create_flow_node(self, x, y, title="New Node"):
        node_id = len(self.flow_nodes)
        ...
        def on_delete_click(e):
            ...
            if e.control in self.flow_nodes:
                self.flow_nodes.remove(e.control)
    ...
    def _add_flow_node(self, e):
        # Add a new node at a random position (in a real app, this might be at the center or mouse position)
        import random
        x = random.randint(50, 300)
        y = random.randint(50, 200)
        node = self._create_flow_node(x, y, f"Node {len(self.flow_nodes) + 1}")
        self.flow_canvas_ref.current.content.controls.append(node)
        self.page.update()

    def _clear_flow_canvas(self, e):
        self.flow_nodes.clear()
        self.flow_connections.clear()
        self.selected_node = None
        self.flow_canvas_ref.current.content.controls = [ft.Container(
            content=None,
            width=float('inf'),
            height=float('inf'),
            bgcolor=ft.Colors.GREY_50
    ...
    def _save_flow(self, e):
        self.page.add(ft.Text(f"Flow saved with {len(self.flow_nodes)} nodes and {len(self.flow_connections)} connections"))
        self.page.update()
                            ...
                            ft.Container(
                                content=ft.Row([
                                    ft.ElevatedButton(
                                        "Add Node",
                                        icon=ft.Icons.ADD,
                                        on_click=self._add_flow_node
                                    ),
                                    ft.ElevatedButton(
                                        "Clear Canvas",
                                        icon=ft.Icons.CLEAR,
                                        on_click=self._clear_flow_canvas
                                    ),
                                    ft.ElevatedButton(
                                        "Save Flow",
                                        icon=ft.Icons.SAVE,
    ...
    def _add_initial_flow_node(self):
        """添加初始流程节点"""
        if self.flow_canvas_ref.current and len(self.flow_canvas_ref.current.content.controls) == 1:
            # Only the background container exists, add initial node
            initial_node = self._create_flow_node(100, 100, "Start")
